...
template <typename READ_COL_TYPE, typename DATA_TYPE, size_t in_buff_len = 4>
__device__
void compression_init_warp_orig(queue<DATA_TYPE>& rq, uint64_t* col_len_ptr, uint64_t* blk_offset_ptr, uint64_t CHUNK_SIZE, int COMP_COL_LEN) {
...
__device__ void write_byte_op_orig(int8_t *out_buffer,uint8_t write_byte, uint64_t *out_offset_ptr) {
  //if(blockIdx.x == 1 && threadIdx.x == 0) printf("val: %u\n", write_byte);
...
template <typename INPUT_T>
__device__ void write_varint_op_orig(int8_t *out_buffer, INPUT_T val, uint64_t *out_offset_ptr) {
  INPUT_T write_val = val;
  int8_t write_byte = 0;
  do {
    write_byte = write_val & 0x7F;
...
template <typename READ_COL_TYPE, typename DATA_TYPE, typename DECOMP_TYPE, size_t in_buff_len = 4>
__device__
void compression_warp_orig( int8_t *out_buffer, queue<DATA_TYPE>& rq, queue<write_queue_ele>& wq, uint64_t* col_len_ptr, uint64_t* blk_offset_ptr, uint64_t CHUNK_SIZE, int COMP_COL_LEN) {
    ...
    while(used_bytes < my_chunk_size) {
        ...
        if(prev_val + cur_delta == read_data && delta_flag){
            ...
            //max
                ...
                write_varint_op_orig<DATA_TYPE>(out_buffer_ptr, delta_first_val, &out_offset);
                delta_count = 1;
                data_buffer_count = 0;
                lit_count = 0;
            }
...
template <typename READ_COL_TYPE, typename DATA_TYPE, typename OUT_COL_TYPE, uint16_t in_queue_size = 4>
__global__ void 
setup_deflate_orig(uint8_t* input_ptr,  uint64_t*  col_len_ptr, uint64_t*  blk_offset_ptr, uint64_t CHUNK_SIZE, int COMP_COL_LEN){
    ...
    if (threadIdx.y == 0) {
        ...
        compression_reader_warp<READ_COL_TYPE, DATA_TYPE > (in_queue, (DATA_TYPE*) chunk_ptr, CHUNK_SIZE * 32, CHUNK_SIZE);
    }

    else if (threadIdx.y == 1) {
        queue<DATA_TYPE> in_queue(in_queue_[threadIdx.x], h + threadIdx.x, t + threadIdx.x, in_queue_size);
        compression_init_warp_orig<READ_COL_TYPE, DATA_TYPE, in_queue_size >(in_queue, col_len_ptr, blk_offset_ptr, CHUNK_SIZE, COMP_COL_LEN);
    }
...
template <typename READ_COL_TYPE, typename DATA_TYPE, typename OUT_COL_TYPE, uint16_t queue_size = 4>
__global__ void 
deflate_orig(uint8_t* input_ptr,  uint8_t* out,  uint64_t*  col_len_ptr, uint64_t*  blk_offset_ptr, uint64_t CHUNK_SIZE, int COMP_COL_LEN){
    ...
    if (threadIdx.y == 0) {
        ...
        queue<DATA_TYPE> in_queue(in_queue_[threadIdx.x], h + threadIdx.x, t + threadIdx.x, queue_size);
        queue<write_queue_ele> out_queue(out_queue_[threadIdx.x], out_h + threadIdx.x, out_t + threadIdx.x, queue_size);
        compression_warp_orig<READ_COL_TYPE, DATA_TYPE, OUT_COL_TYPE, queue_size >((int8_t*)out, in_queue, out_queue, col_len_ptr, blk_offset_ptr, CHUNK_SIZE, COMP_COL_LEN);
    }
    __syncthreads();
}
