...
// #define FUNNEL_TYPE uint32_t
// #define USE_EIGHT 0

#define NMEMCPY 8 // Must be power of 2, acceptable = {1,2,4}
...
template<typename T>
__device__
void write_literal_8(T* out_ptr, uint32_t* counter, uint8_t idx, uint8_t b){
    if(threadIdx.x == idx){
        out_ptr[counter] = b;
        *counter++;
    }
...
// __forceinline__ 
__device__
void memcpy_onebyte_8(T* out_ptr, uint32_t* counter, uint8_t idx, uint16_t len, uint16_t offset, uint8_t div, uint32_t MASK) {
...
// __forceinline__ 
__device__
void memcpy_nbyte_prefix_body_suffix_8(T* out_ptr, uint32_t* counter, uint8_t idx, uint16_t len, uint16_t offset, uint8_t div, uint32_t MASK) {
    // if (len <= 32) {
    int tid;
...
// __forceinline__ 
__device__
void memcpy_nbyte_prefix_body_suffix_needs_writing_alignment_8(T* out_ptr, uint32_t* counter, uint8_t idx, uint16_t len, uint16_t offset, uint8_t div, uint32_t MASK) {
    // if (len <= 32) {
    int tid;
    uint32_t orig_counter;
    uint8_t num_writes;
...
template <uint32_t NUM_THREAD = 8,
    ...
    __device__
    void memcpy_nbyte_prefix_body_suffix_funnelshift_shared_8(T* out_ptr, uint32_t* counter, TYPE_EXT* arr_shared, uint8_t idx, uint16_t len, uint16_t offset, uint8_t div, uint32_t MASK) {
...
// __forceinline__ 
__device__
void memcpy_nbyte_prefix_body_suffix_shared_8(T* out_ptr, uint32_t* counter, T* arr_shared, uint8_t idx, uint16_t len, uint16_t offset, uint8_t div, uint32_t MASK) {
    // if (len <= 32) {
    int tid;
    uint32_t orig_counter;
    uint8_t num_writes;
...
// __forceinline__ 
__device__
void memcpy_nbyte_prefix_body_suffix_needs_writing_alignment_shared_8(T* out_ptr, uint32_t* counter, T* arr_shared, uint8_t idx, uint16_t len, uint16_t offset, uint8_t div, uint32_t MASK) {
    int tid;
    uint32_t orig_counter;
    uint8_t num_writes;
    uint32_t start_counter;
...
// __forceinline__ 
__device__
void memcpy_nbyte_aligned_8(T* out_ptr, uint32_t* counter, uint8_t idx, uint16_t len, uint16_t offset, uint8_t div, uint32_t MASK) {
    int tid = threadIdx.x - div * NUM_THREAD;
    uint32_t orig_counter = __shfl_sync(MASK, *counter, idx);
    ...
    #if NMEMCPY == 1
    uint8_t* out_ptr_temp   = out_ptr;
    #elif NMEMCPY == 2
    uint16_t* out_ptr_temp  = reinterpret_cast<uint8_t*>(out_ptr);
    #elif NMEMCPY == 4
