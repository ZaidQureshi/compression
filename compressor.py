...
def zlib_strided_compress(data):
    ...
    ret = [];
    output_f_test2 = open("output/compressed_file.bin", 'wb');
        ...
        for k in chunks:
            ...
            for p in range(n_chunks):
                ...
                for j in range(WARP_SIZE):
                    ...
                    #f_nd = np.concatenate((nd, nd2), axis=0);
                    #f_nd = f_nd.T
                    #f_nd = f_nd.flatten();
                    #nd = (data_copy_r[(j*2,j*2+1)::(WARP_SIZE*2)]).reshape((-1))
                    f_nd = [];
                    ...
                    input.append(nd);
                    #cctx = zstd.ZstdCompressor(level=level);
                    zc = zlib.compress(nd, level=2);
                    zc_array_temp.append(list(zc));
                    zc_len_array_temp.append(len(list(zc)));
                    # if (p == 1 and j == 0):
                    #  #   output_f_test = open("output/comp", 'wb');
                    #   #  output_f_test.write(zc);
                    #    # t =(0xaabb)
                    #    # b = (t).to_bytes(2, byteorder='big');
                    #    #? zc2 = zlib.compress(b, level=6);
                    #    #? print("b1: ",  binascii.hexlify(b));
                    #    print("nd 1: ", binascii.hexlify(nd1))
                    #    print("nd 2: ", binascii.hexlify(nd2));
                    #    print("zc1: ",  binascii.hexlify(nd));
                ...
                for i2 in range(32):
                    len_array.append(0);
                m_len = max(zc_len_array[c2]);
                max_len_4B = int((m_len+max_len - 1)/max_len);
                ...
                for i2 in range(max_len_4B):
                    for j2 in range(32):
                        ...
                        if remain > 0:
                            for k2 in range(max_len):
                                c_write = c_write + 1;
                                if(remain > 0 ):
                                    b = zc_array[c2][j2][i2*max_len+k2];
                ...
                for i3 in range(32):
                    l = zc_len_array[0][i3];
                    l = int((l+max_len - 1)/max_len)*max_len;
                    blk_len = blk_len + l;
